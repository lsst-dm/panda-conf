#!/bin/bash
#
# pilot3 wrapper used at CERN central pilot factories
#
# https://google.github.io/styleguide/shell.xml

VERSION=20200528a-pilot3
...
function pilot_cmd() {
  ...
  # fi
  # cmd="${pybin} pilot3/pilot.py -q ${qarg} -i ${iarg} -j ${jarg} ${opt_harvester_datadir} --pilot-user=rubin ${pilotargs}"

  # The option x was added in pilot-2.12.4.7
  # set the maximum failures to 3 on getjob request
  # set the maximum of getjob requests to 5
  #
  cmd="${pybin} pilot3/pilot.py -q ${qarg} -i ${iarg} -j ${jarg} --pilot-user=rubin -x 3 -v 5 ${pilotargs}"
...
function get_pilot() {
  ...
  if [[ ${harvesterflag} == 'true' ]] && [[ ${workflowarg} == 'OneToMany' ]]; then
    cp -v ${HARVESTER_WORK_DIR}/pilot3.tar.gz .
  fi
  ...
  if [[ -f pilot3/pilot.py ]]; then
    log "File pilot3/pilot.py exists OK"
    log "pilot3/PILOTVERSION: $(cat pilot3/PILOTVERSION)"
    return 0
  else
    log "ERROR: pilot3/pilot.py not found"
    err "ERROR: pilot3/pilot.py not found"
    return 1
  fi
}
...
function main() {
  ...
  echo "This is PanDA pilot3 wrapper version: $VERSION"
  ...
  if [[ -f ${workdir}/pilot3/pandaIDs.out ]]; then
    # max 30 pandaids
    pandaids=$(cat ${workdir}/pilot3/pandaIDs.out | xargs echo | cut -d' ' -f-30)
    log "pandaids: ${pandaids}"
  else
    log "File not found: ${workdir}/pilot3/pandaIDs.out, no payload"
    err "File not found: ${workdir}/pilot3/pandaIDs.out, no payload"
    pandaids=''
  fi
...
# additional pilot3 args are passed as extra args
containerflag='false'
containerarg=''
harvesterflag='false'
harvesterarg=''
